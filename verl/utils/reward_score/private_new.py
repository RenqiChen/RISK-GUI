...
def format_reward_private_new(completions, reward_coeffs, process_ids):
...
def compute_score(prediction, ground_truth, extra_info, global_steps):
    ...
    format_coeff = 0.1
    accuracy_coeff = 0.9
    score = format_reward_private_new(prediction, format_coeff, process_id) + action_reward_private(prediction, ground_truth, accuracy_coeff, process_id, step_id)
    return score
